...
class TestHttpServer:
    ...
    def server_client(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(server, "INTERNAL_TOKEN", "test-token-minimum-16chars")
    ...
    def test_start_rejects_private_callback_resolution(self, server_client, monkeypatch, tmp_path):
        ...
        response = client.post("/sprints", json=payload, headers={"X-Internal-Token": "test-token-minimum-16chars"})
    ...
    def test_start_list_get_and_stop_sprint(self, mock_stop, mock_spawn, server_client, tmp_path):
        ...
        list_response = client.get("/sprints", headers={"X-Internal-Token": "test-token-minimum-16chars"})
        assert list_response.status_code == 200
        assert any(s["sprint_id"] == sprint_id for s in list_response.json())

        get_response = client.get(f"/sprints/{sprint_id}", headers={"X-Internal-Token": "test-token-minimum-16chars"})
        assert get_response.status_code == 200
        assert get_response.json()["project"] == "demo"

        stop_response = client.post(f"/sprints/{sprint_id}/stop", headers={"X-Internal-Token": "test-token-minimum-16chars"})
